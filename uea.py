...
def load_UEA_dataset(path, dataset):
    ...
    weka.core.jvm.start()
    loader = weka.core.converters.Loader(classname="weka.core.converters.ArffLoader")
...
def fit_hyperparameters(file, train, train_labels, cuda, gpu, save_memory=False):
    """
    ...
    return classifier.fit(train, train_labels, save_memory=save_memory, verbose=True)
...
if __name__ == "__main__":
    ...
    if not args.load and not args.fit_classifier:
        ...
        classifier = scikit_wrappers.CausalCNNEncoderClassifier()
        hf = open(
            os.path.join(args.save_path, args.dataset + "_hyperparameters.json"), "r"
        )
    ...
    if not args.load:
        ...
        classifier.save(os.path.join(args.save_path, args.dataset))
        with open(
            os.path.join(args.save_path, args.dataset + "_hyperparameters.json"), "w"
        ) as fp:
