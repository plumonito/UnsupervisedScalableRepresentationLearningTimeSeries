...
def parse_arguments():
    ...
    parser.add_argument(
        "--dataset", type=str, metavar="D", required=True, help="dataset name"
    )
    parser.add_argument(
        "--path",
        type=str,
    ...
    parser.add_argument(
        ...
        required=True,
        nargs="+",
        help="list of folders, each one containing a model " + "for the chosen dataset",
    )
    parser.add_argument(
        "--save_path",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="activate to load the classifier instead of " + "training it",
    )
    parser.add_argument("--cuda", action="store_true", help="activate to use CUDA")
    parser.add_argument(
        "--gpu",
        type=int,
        default=0,
...
if __name__ == "__main__":
    ...
    classifiers = [
        load_classifier(
            os.path.join(args.model_path, folder), args.dataset, args.cuda, args.gpu
        )
        for folder in args.folders
    ]

    train_representations = numpy.concatenate(
        [c.encode(train) for c in classifiers], axis=1
    )
    test_representations = numpy.concatenate(
        [c.encode(test) for c in classifiers], axis=1
    )

    classifier = sklearn.svm.SVC(C=numpy.inf, gamma="scale")

    if not args.load:
        nb_classes = numpy.shape(numpy.unique(train_labels, return_counts=True)[1])[0]
        ...
        sklearn.externals.joblib.dump(
            classifier,
            os.path.join(args.save_path, args.dataset + "_CausalCNN_classifier.pkl"),
        )
    else:
        classifier = sklearn.externals.joblib.load(
            os.path.join(args.save_path, args.dataset + "_CausalCNN_classifier.pkl")
        )

    # Testing
    print("Test accuracy: " + str(classifier.score(test_representations, test_labels)))
